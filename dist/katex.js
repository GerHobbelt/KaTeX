...
function parseArray(parser, result, style) {
    ...
    while (true) {
        ...
        if (next === "&") {
            ...
            if (body.length > 1 && lastRow.length === 1 && lastRow[0].value.length === 0) {
                body.pop();
            }
            break;
...
(0, _defineFunction2.default)({
    ...
    htmlBuilder: function htmlBuilder(group, options) {
        ...
        if (group.value.value.length > 0) {
            ...
            var temp = html.buildExpression(group.value.value, options.withFont("mathrm"), true);
        ...
        var span = _buildCommon2.default.makeSpan(["mop"], output, options);
        return span;
    },
    ...
    mathmlBuilder: function mathmlBuilder(group, options) {
        ...
        var output = [];
        if (group.value.value.length > 0) {
            var temp = mml.buildExpression(group.value.value, options.withFont("mathrm"));

            var word = "";
            for (var i = 0; i < temp.length; i++) {
                if (_utils2.default.contains(["mi", "mn", "mo"], temp[i].type)) {
                    ...
                    if (temp[i].lexer) {
                        var pos = temp[i].lexer.pos;
                        throw new Error("unrecognized group in \\operatorname at " + pos);
                    }
                }
            }
            word = word.replace(/\u2212/g, "-");
...
},{"../buildCommon":91,"../buildHTML":92,"../buildMathML":93,"../defineFunction":96,"../domTree":98,"../mathMLTree":108,"../utils":115}],106:[function(require,module,exports){
"use strict";
...
defineSymbol(math, main, mathord, "\xA3", "\\pounds", true);
...
for (var _i2 = 0; _i2 < extendedLatinBase.length; _i2++) {
    var _ch2 = extendedLatinBase.charAt(_i2);
...
for (var _i4 = 0; _i4 < letters.length; _i4++) {
    var _ch4 = lowercaseGreekLetters.charAt(_i4);
    defineSymbol(math, main, mathord, _ch4, _ch4);
}

var uppercaseGreekLetters = "ΓΔΘΞΠΣΦΨΩ";
for (var _i5 = 0; _i5 < letters.length; _i5++) {
    var _ch5 = uppercaseGreekLetters.charAt(_i5);
    defineSymbol(math, main, textord, _ch5, _ch5);
}

// Latin-1 letters
for (var _i6 = 0x00C0; _i6 <= 0x00D6; _i6++) {
    var _ch6 = String.fromCharCode(_i6);
    defineSymbol(math, main, mathord, _ch6, _ch6);
    defineSymbol(text, main, textord, _ch6, _ch6);
}

for (var _i7 = 0x00D8; _i7 <= 0x00F6; _i7++) {
    var _ch7 = String.fromCharCode(_i7);
    defineSymbol(math, main, mathord, _ch7, _ch7);
    defineSymbol(text, main, textord, _ch7, _ch7);
}

for (var _i8 = 0x00F8; _i8 <= 0x00FF; _i8++) {
    var _ch8 = String.fromCharCode(_i8);
    defineSymbol(math, main, mathord, _ch8, _ch8);
    defineSymbol(text, main, textord, _ch8, _ch8);
}

// Cyrillic
for (var _i9 = 0x0410; _i9 <= 0x044F; _i9++) {
    var _ch9 = String.fromCharCode(_i9);
    defineSymbol(text, main, textord, _ch9, _ch9);
}
